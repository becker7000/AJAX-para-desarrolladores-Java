...
@WebServlet("/formulario-avanzado")
public class ValidarFormularioAServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if( password==null || password.isBlank() ){
            errores.put("password","La contraseña de usuario es requerida");
        }
        if( email==null || !email.contains("@") ){
            errores.put("email","El correo no es valido o sin correo");
        }
        if(country==null || country.isBlank()){
            errores.put("country","El país del usuario es requerido");
        }
        if(languages==null || languages.length==0){
            errores.put("languages","Selecciona al menos un lenguaje");
        }
        if(idiomas==null || idiomas.length==0){
            errores.put("idiomas","Selecciona al menos un idioma");
        }
        ...
        try(PrintWriter salida = resp.getWriter()) {
            if(errores.isEmpty()){
                ...
                salida.println("    </head>");
                salida.println("    <body>");
                salida.println("        <h1>Validando formulario avanzado</h1>");
                ...
                getServletContext()
                        .getRequestDispatcher("/formularioAvanzado.jsp")
                        .forward(req,resp);
            }
        }
